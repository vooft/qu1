...
QNetworkRequest O1Requestor::setup(const QNetworkRequest &req, const QList<O1RequestParameter> &signingParameters, QNetworkAccessManager::Operation operation) {
    ...
    oauthParams.append(O1RequestParameter(O2_OAUTH_SIGNATURE_METHOD, O2_SIGNATURE_TYPE_PLAINTEXT));
    ...
    QByteArray signature = QString(authenticator_->clientSecret()).append("&").append(authenticator_->tokenSecret()).toLatin1();
